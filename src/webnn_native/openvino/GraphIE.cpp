...
namespace webnn_native { namespace ie {
    ...
    namespace {
        enum InputLayout { Nchw, Nhwc, Hwnc };
        enum TransposeType { NhwcToNchw, HwncToNchw, NchwToNhwc, NchwToHwnc };
...
#define SLICE_ONE_AXIS(axis, index)           \
    ...
    MaybeError Graph::AddResample2d(const op::Resample2d* resample2d) {
        ...
        // sizes.
        auto output = resample2d->Outputs()[0].Get();
        ...
        if (inputLayout == InputLayout::Hwnc) {
            resampleNode = TransposeInputLayout(resampleNode, TransposeType::NchwToHwnc);
        } else if (inputLayout == InputLayout::Nhwc) {
            resampleNode = TransposeInputLayout(resampleNode, TransposeType::NchwToNhwc);
        }
